...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // Reserve stock with atomic conditional decrement so two concurrent
    // orders for the same product cannot both succeed past the available
    // quantity. The check-then-write pattern from before could oversell.
    let subtotal = 0;
    const orderItems = [];
    ...
    const deliveryFee = await calculateShippingTotal({
      city: validatedData.shipping.city,
      isInternational,
      subtotal,
      shippingMethodCode: validatedData.shippingMethod,
    });
    ...
    const order = new Order({
      ...
      payment: {
        ...
        provider: "paystack",
        reference: `REF-${Date.now()}-${randomBytes(8).toString("hex")}`,
        status: "pending",
        amount: total,
      },
      status: "pending",
