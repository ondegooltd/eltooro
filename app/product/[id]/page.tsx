...
export async function generateMetadata({ params }: ProductPageProps): Promise<Metadata> {
  ...
  const price = typeof product.price === "object" ? product.price.ghs : product.price
  const images = product.images?.map((img: { url: string }) => img.url) || []
  const mainImage = images[0] || "/placeholder-product.jpg"

  return generateProductMetadata({
    name: product.name,
    description: product.description || product.shortDescription || `${product.name} - Organic beauty product from Eltooro. Fast delivery in Ghana.`,
    image: mainImage,
    slug: product.slug,
    price,
    currency: "GHS",
