...
// Always render with fresh data so admin changes to cities / shipping
// methods appear immediately.
...
const formatCedi = (value: number) =>
  `GH₵${Number.isInteger(value) ? value : value.toFixed(2)}`;

async function getShippingRates() {
  const db = await getDb();

  const [settings, methods] = await Promise.all([
    db.collection("adminsettings").findOne({}),
    db
  ...
  const cities = (settings?.deliveryFees?.cities ?? []) as DeliveryCity[];

  // For each delivery city, derive each delivery method's price from the
  // city's base fee × the method's multiplier.
  return cities.map((city) => ({
    area: city.name,
    options: methods.map((method) => ({
      name: method.name as string,
      time: method.deliveryTime as string,
      price: formatCedi(city.fee * (method.multiplier ?? 1)),
    })),
  }));
}
          ...
          <div className="container mx-auto px-4">
            ...
            {shippingRates.length === 0 && (
              <p className="text-center text-muted-foreground">
                Shipping rates are being updated. Please check back soon.
              </p>
            )}
