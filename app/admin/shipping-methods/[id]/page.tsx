...
export default function AdminShippingMethodEditPage() {
  ...
  const fetchShippingMethod = async () => {
    try {
      ...
      if (!response.ok) throw new Error("Failed to fetch delivery method");
      ...
      toast({
        title: "Error",
        description: "Failed to load delivery method",
        variant: "destructive",
      });
    } finally {
      setIsLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          error.error?.message || "Failed to update delivery method",
        );
      }
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(
          error.error?.message || "Failed to delete delivery method",
        );
      }
    ...
    return (
      <ProtectedRoute requireAdmin>
        <div className="min-h-screen flex items-center justify-center">
          <div>Loading delivery method...</div>
        </div>
      </ProtectedRoute>
    );
  }
  ...
  if (!shippingMethod) {
    return (
      <ProtectedRoute requireAdmin>
        <div className="min-h-screen flex items-center justify-center">
          <div>Delivery method not found</div>
        </div>
      </ProtectedRoute>
    );
  }
          ...
          <div className="container mx-auto px-4 py-8">
            <div className="mb-6">
              ...
              <div className="flex justify-between items-center">
                <h1 className="text-3xl font-bold">Edit Delivery Method</h1>
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-4">
                      <div>
                        ...
                        />
                        <p className="text-xs text-muted-foreground mt-1">
                          Multiplier for base shipping cost (e.g., 1.0 for
                          standard, 1.5 for express)
                        </p>
                      </div>
                      <div>
                      ...
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            Show delivery method on checkout
                          </p>
                        </div>
                        <Switch
