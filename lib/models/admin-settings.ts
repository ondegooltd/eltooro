...
export interface IAdminSettingsDeliveryCity {
  name: string;
  fee: number;
}

export interface IAdminSettingsDeliveryFees {
  // Admin-managed list of cities/towns and their delivery fees (CRUD)
  cities: IAdminSettingsDeliveryCity[];
  // Flat fee charged for any city not present in `cities`
  other: number;
...
const DeliveryCitySchema = new Schema<IAdminSettingsDeliveryCity>(
  {
...
const DeliveryFeesSchema = new Schema<IAdminSettingsDeliveryFees>(
  {
    cities: { type: [DeliveryCitySchema], required: true, default: [] },
    other: { type: Number, required: true, min: 0 },
