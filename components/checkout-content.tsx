...
export function CheckoutContent() {
  ...
  // Admin-managed delivery cities (drives the city dropdown and fee lookup)
  const deliveryCities: DeliveryCity[] =
    deliveryFees?.deliveryFees?.cities ?? [];
  const otherCityFee: number =
    deliveryFees?.deliveryFees?.other ?? OTHER_CITY_DEFAULT_FEE;
  ...
  // True when the customer's city is not in the listed cities and they're
  // entering it manually (charged the configurable "other" delivery fee).
  ...
  const loadUserData = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        // If the saved city isn't one of the listed cities, switch the
        // dropdown into "Other" mode so the value is preserved and editable.
        const savedCity = user.addresses?.find(
          (a: any) => a.isDefault && a.type === "shipping",
        )?.city;
        setIsOtherCity(
          Boolean(savedCity) && !isListedCity(deliveryCities, savedCity),
        );
  ...
  const calculateShippingCost = () => {
    ...
    if (isOtherCity) return otherCityFee;
    return findCityFee(deliveryCities, shippingData.city) ?? otherCityFee;
  };
  ...
  // Free standard shipping above the threshold applies ONLY to listed cities,
  // never to unlisted ("Other") destinations.
  const freeShippingEligible =
    !isOtherCity &&
    isListedCity(deliveryCities, shippingData.city) &&
  ...
  const handleShippingSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const hasCity = isOtherCity
      ? shippingData.city.trim().length > 0
      : isListedCity(deliveryCities, shippingData.city);
    if (!hasCity) {
      toast({
        title: "Error",
        description: isOtherCity
              ...
              <div className="bg-white rounded-lg border border-border p-4 sm:p-6">
                ...
                <div className="grid sm:grid-cols-3 gap-4 mb-4">
                  <div>
                    ...
                    <Select
                      value={isOtherCity ? OTHER_CITY_VALUE : shippingData.city}
                      onValueChange={(value) => {
                ...
                {isLoadingShippingMethods ? (
                  ...
                  >
                    {shippingMethods.map((method, index) => {
                      const methodCost =
                        calculateShippingCost() * method.multiplier;
